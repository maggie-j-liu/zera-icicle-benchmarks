...
auto prove_sumcheck(SumcheckTranscript<scalar_t> &pt, scalar_t* a, int log_mle_poly_size, scalar_t claim, SumcheckProof<scalar_t>& proof)
    -> std::vector<scalar_t> {
	proof.init(log_mle_poly_size, 1);
    ...
    for (size_t round = 0; round < log_mle_poly_size; round++, half /= 2) {
        ...
        //   p0 = g(0)
        //   p1 = g(1)
        // where g(x) = sum_{rest} F(x,rest) and deg(g)=1.

        ScalarAddReducer p0{0}, p1{0};

        cilk_gpu_for (size_t j = 0; j < half; j++) {
            p0 = p0 + a[j];           // x = 0 half
            p1 = p1 + a[j + half];    // x = 1 half
        }
        ...
        // Bind top variable to r
        bound_top_var(a, size, *r_);
		remaining_log_poly_size--;
        sync_current_stream();

        // Since g(x) = p0 + (p1 - p0)x
        claim = p0 + (p1 - p0) * r;
    }
...
double run_benchmark_once(int log_mle_poly_size) {
	...
	int combine_func_poly_degree = 1; 
	...
	ctimer_t t;
	ctimer_start(&t);
	prove_sumcheck(sumcheck_transcript, mle_polynomials[0], log_mle_poly_size, claimed_sum, sumcheck_proof);
	ctimer_stop(&t);
	ctimer_measure(&t);
